                                            ...
                                            <div id="card-teaching-mentoring">
                                                ...
                                                <div class="card rounded-6 my-shadow border-0 mt-3 {{ $loop->last ? 'mb-3' : '' }}">
                                                    <div class="card-body">
                                                        ...
                                                        <div class="fw-semibold {{ $item->category == 'Pengajaran' ? 'd-none' : ''  }}" style="font-size: 17px; text-transform: capitalize">{{ $item->title }}</div>
                                                        <div class="fw-semibold mt-0 {{ $item->category == 'Pengajaran' ? '' : 'd-none' }}" style="font-size: 17px; text-transform: capitalize">{{ $item->category == 'Pengajaran' ? $item->subject->name : '' }}</div>
                    ...
                    if (response.status === `success`) {
                        ...
                        response.data.forEach(function(item, index) {
                            var cardHtml = `<div class="card rounded-6 my-shadow border-0 mt-3 ${index === (response.data.length - 1) ? 'mb-3' : ''}">` +
                                `<div class="card-body">` +
                                `<span class="text-uppercase fw-light" style="font-size: 12px">${item.category}</span>` +
                                `<div class="fw-semibold ${item.category === 'Pengajaran' ? 'd-none' : ''}" style="font-size: 17px; text-transform: capitalize">${item.title ? item.title : ''}</div>` +
                                `<div class="fw-semibold mt-0 ${item.category === 'Pengajaran' ? '' : 'd-none'}" style="font-size: 17px; text-transform: capitalize">${item.category === 'Pengajaran' && item.subject ? item.subject.name : ''}</div>` +
                                `<div class="text-muted">${item.student_name ? 'Mahasiswa: ' + item.student_name : ''}</div>` +
                                `<div class="text-muted mt-1 fw-light">${item.year}</div>` +
                                `</div>` +
                                `</div>`;
